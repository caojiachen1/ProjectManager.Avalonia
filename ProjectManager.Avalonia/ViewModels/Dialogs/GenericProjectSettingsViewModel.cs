...
public partial class GenericProjectSettingsViewModel : ViewModelBase
{
    ...
    private async Task BrowseLocalPath()
    {
        ...
        {
            LocalPath = path;
            if (string.IsNullOrEmpty(WorkingDirectory)) WorkingDirectory = path;
            if (string.IsNullOrEmpty(ProjectName)) ProjectName = Path.GetFileName(path);
        }
    }
    ...
    {
        if (!string.IsNullOrEmpty(command)) StartCommand = command;
    }
    ...
    private async Task Save()
    {
        try
        {
            if (string.IsNullOrWhiteSpace(ProjectName)) { await ShowErrorMessage("项目名称不能为空"); return; }
            if (string.IsNullOrWhiteSpace(LocalPath)) { await ShowErrorMessage("项目路径不能为空"); return; }
            ...
            if (!string.IsNullOrWhiteSpace(TagsString))
                project.Tags = TagsString.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(t => t.Trim()).Where(t => !string.IsNullOrEmpty(t)).ToList();
            else
                project.Tags = new List<string>();

            project.LastModified = DateTime.Now;
            if (_originalProject == null) project.CreatedDate = DateTime.Now;

            if (await _projectService.SaveProjectAsync(project))
                ProjectSaved?.Invoke(this, project);
        }
        catch (Exception ex) { await ShowErrorMessage($"保存项目失败: {ex.Message}"); }
    }
    ...
    private async Task Delete()
    {
        if (_originalProject != null)
        {
            ...
            catch (Exception ex) { await ShowErrorMessage($"删除项目失败: {ex.Message}"); }
        }
    }

    private async Task ShowErrorMessage(string message) => await _errorDisplayService.ShowErrorAsync(message, "错误");
}
